import { getHuaweiCredentialAPI } from "@/api";
...
const HuaweiConfigPage = () => {
  const [openedCreateModal, { open: openCreateModal, close: closeCreateModal }] =
    useDisclosure();
  const [credentials, setCredentials] = useState<HuaweiCredential[]>([]);
  const { isLoading, refetch } = useQuery(
    ["get-huawei-credentials"],
    getHuaweiCredentialAPI,
    {
      onSuccess: ({ data }) => {
    ...
    >
      <Stack spacing={0}>
        <Group
          ...
          align="center"
        >
          <Title fw={700}>Huawei Configuration</Title>
...
export default HuaweiConfigPage;
